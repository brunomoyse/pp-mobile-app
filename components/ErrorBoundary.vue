<template>
  <div v-if="hasError" class="pp-error-state">
    ...
    <p class="pp-error-description">
      {{ errorMessage || t('error.boundary.defaultMessage') }}
    </p>

    <div v-if="showDetails && errorDetails" class="pp-error-expandable">
      <button
        class="pp-error-toggle-btn"
        @click="detailsExpanded = !detailsExpanded"
      >
        {{ detailsExpanded ? t('error.boundary.hideDetails') : t('error.boundary.showDetails') }}
      </button>

      <div v-if="detailsExpanded" class="pp-error-stack">
        <pre>{{ errorDetails }}</pre>
      </div>
    </div>

    <div class="pp-error-buttons">
      <button
        class="pp-action-button pp-action-button--retry"
        @click="handleRetry"
      >
...
/* Scrollbar styling */
.pp-error-stack::-webkit-scrollbar {
  width: 4px;
}

.pp-error-stack::-webkit-scrollbar-track {
  background: #18181a;
  border-radius: 2px;
}

.pp-error-stack::-webkit-scrollbar-thumb {
  background: #54545f;
  border-radius: 2px;
}

.pp-error-stack::-webkit-scrollbar-thumb:hover {
  background: #fee78a;
}
</style>
